...
if (isset($_POST["search"]["value"])) {
    $query1 = '(id LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR Username LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR EmailAddress LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR Password LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR UserMobile LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR role LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR image LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR Hospital_Name LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR Qualification LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR Experience LIKE "%' . $_POST["search"]["value"] . '%"';
    $query1 .= 'OR Address LIKE "%' . $_POST["search"]["value"] . '%"';
     $query1 .= 'OR Pincode LIKE "%' . $_POST["search"]["value"] . '%"';
     $query1 .= 'OR fee LIKE "%' . $_POST["search"]["value"] . '%"';
     $query1 .= 'OR status LIKE "%' . $_POST["search"]["value"] . '%"';
...
if (isset($_POST["order"])) {
 ...
 ';
    ...
    $query1 .= 'ORDER BY last_updated Desc ';
}
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $sub_array[] = '<div style="text-align:center;color:black">' . $row["id"] . '</div>';
    ...
    $sub_array[] = '<div style="text-align:center;color:black">' . $row["EmailAddress"] . '</div>';
     $sub_array[] = '<div style="text-align:center;color:black">' . $row["Password"] . '</div>';
    ...
    $sub_array[] = '<div style="text-align:center;color:black">' . $row["status"] . '</div>';
        E$sub_array[] = '<a href="edit-staff.php?id='.$row["id"].'"><i class="fas fa-edit fa-2x" id="update"></i></a>';
         $data[] = $sub_array;
    $i++;
}
